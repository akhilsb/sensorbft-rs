    ...
    pub(crate) async fn process_msg(self: &mut Context, wrapper_msg: WrapperMsg){
        ...
        if self.check_proposal(msg){
            ...
            match wrapper_msg.clone().protmsg {
                CoinMsg::PedAVSSInit(share,verifier,ctr)=>{
                    log::debug!("Received AVSS init {:?} from node {}",share,ctr.origin);
                    self.process_wss_init(share,verifier,ctr).await;
                },
                CoinMsg::PedAVSSEcho(ctr, echo_sender)=>{
                    log::debug!("Received AVSS ECHO from node {} for secret from {}",echo_sender,ctr.origin);
                    self.process_wssecho( ctr,echo_sender).await;
                },
                CoinMsg::PedAVSSReady(ctr, ready_sender)=>{
                    log::debug!("Received AVSS READY from node {} for secret from {}",ready_sender,ctr.origin);
                    self.process_wssready(ctr,ready_sender).await;
                },
                CoinMsg::PedAVSSReconstruct(ctr, recon_sender)=>{
                    log::debug!("Received AVSS Recon from node {} for secret from {}",recon_sender,ctr.origin);
                    self.process_wssrecon(ctr,recon_sender).await;
                },
                CoinMsg::PedAVSSSRecon(share, ss_sender,ss_init )=>{
                    log::debug!("Received AVSS Secret recon from node {} for secret from {}",ss_sender,ss_init);
                    self.process_reconstruct(share,ss_sender,ss_init).await;
                },
